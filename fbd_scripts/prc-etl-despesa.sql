...
BEGIN
    ...
    DECLARE curs CURSOR FOR 
    	...
    	WHERE COD_DOCUMENTO NOT IN (
    		SELECT COD_DOCUMENTO FROM DESPESA
    	);
    ...
    WHILE (done != true)DO
       ...
       -- Verifica se o Senador já está na base, para recuperar o ID dele 
		SELECT ID_SENADOR INTO idSenador FROM SENADOR WHERE NOME = UPPER(v_senador);
		IF (idSenador IS NOT NULL AND idSenador > 0) THEN
			...
			-- Verifica se o fornecedor já está na base. Se não estiver, insere
				...
				SELECT ID_FORNECEDOR INTO idFornecedor FROM FORNECEDOR WHERE CPF_CNPJ = cpfCnpjLimpo ;
			...
			SELECT ID_TIPO_DESPESA INTO idTipoDespesa FROM TIPO_DESPESA WHERE UPPER(DESCRICAO) = TRIM(UPPER(v_tipoDespesa));
