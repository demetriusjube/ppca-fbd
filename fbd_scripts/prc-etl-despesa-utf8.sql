...
BEGIN
    ...
    DECLARE curs CURSOR FOR 
    	...
    	WHERE COD_DOCUMENTO NOT IN (
    		SELECT COD_DOCUMENTO COLLATE utf8_general_ci AS COD FROM DESPESA
    	) AND SENADOR IN (
    		SELECT DISTINCT s.NOME COLLATE utf8_general_ci AS NOME_SENADOR FROM SENADOR s);
    ...
    # WHILE (done != true)DO
       ...
       # Verifica se o Senador já está na base, para recuperar o ID dele
		...
		SELECT ID_SENADOR INTO idSenador FROM SENADOR WHERE UPPER(NOME) = UPPER(v_senador) COLLATE utf8_general_ci;
		...
		IF (idSenador IS NOT NULL AND idSenador > 0) THEN
			...
			# Verifica se o fornecedor já está na base. Se não estiver, insere
				...
				SELECT ID_FORNECEDOR INTO idFornecedor FROM FORNECEDOR WHERE TRIM(CPF_CNPJ) = cpfCnpjLimpo COLLATE utf8_general_ci;
			...
			SELECT ID_TIPO_DESPESA INTO idTipoDespesa FROM TIPO_DESPESA WHERE TRIM(UPPER(DESCRICAO)) = TRIM(UPPER(v_tipoDespesa)) COLLATE utf8_general_ci;
